...
class Validation
{
    ...
    {
        return in_array(rtrim(explode('*', $attributeKey)[0], '.'), ['', '0'], true) ? null : rtrim(explode('*', $attributeKey)[0], '.');
    }
     ...
     * @return void
     */
    protected function addError(Attribute $attribute, mixed $value, Rule $ruleValidator)
    {
     ...
     * @return void
     */
    protected function setValidData(Attribute $attribute, mixed $value)
    {
        $key = $attribute->getKey();
     ...
     * @return void
     */
    protected function setInvalidData(Attribute $attribute, mixed $value)
    {
        $key = $attribute->getKey();
        if ($attribute->isArrayAttribute() || $attribute->isUsingDotNotation()) {
